...
void Svcmd_BotList_f( void ) {
	...
	for (i = 0; i < g_numBots; i++) {
		...
		Q_strncpyz(name, Info_ValueForKey( g_botInfos[i], "name" ), sizeof(name) ); // fix ioq3 - Cowcat
		if ( !*name ) {
			strcpy(name, "UnnamedPlayer");
		}
		//strcpy(funname, Info_ValueForKey( g_botInfos[i], "funname" ));
		Q_strncpyz(funname, Info_ValueForKey( g_botInfos[i], "funname" ), sizeof(funname) ); // fix ioq3 - Cowcat
		if ( !*funname ) {
			strcpy(funname, "");
		}
		//strcpy(model, Info_ValueForKey( g_botInfos[i], "model" ));
		Q_strncpyz(model, Info_ValueForKey( g_botInfos[i], "model" ), sizeof(model) ); // fix ioq3 - Cowcat
		if ( !*model ) {
			strcpy(model, "visor/default");
		}
		//strcpy(aifile, Info_ValueForKey( g_botInfos[i], "aifile"));
		Q_strncpyz(aifile, Info_ValueForKey( g_botInfos[i], "aifile" ), sizeof(aifile) ); // fix ioq3 - Cowcat
		if (!*aifile ) {
