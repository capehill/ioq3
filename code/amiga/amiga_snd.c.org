...
qboolean SNDDMA_Init(void)
{
	...
	*/
	...
	if ((AHImp = IExec->CreateMsgPort()) == NULL) 
	{
		Com_Printf("ERROR: Can't create AHI message port\n");
		return qfalse;
	}

	if ((AHIio = (struct AHIRequest *)IExec->CreateIORequest(AHImp, sizeof(struct AHIRequest))) == NULL) 
	{
	...
	if ((AHIDevice = IExec->OpenDevice("ahi.device", AHI_NO_UNIT, (struct IORequest *)AHIio, 0)) != 0) 
	{
	...
	if ((actrl = IAHI->AHI_AllocAudio(AHIA_AudioID, AHI_DEFAULT_ID,
					AHIA_MixFreq, 	speed,
					AHIA_Channels, 	1,
					AHIA_Sounds, 	1,
				TAG_END)) == NULL) 
	...
	IAHI->AHI_GetAudioAttrs(AHI_INVALID_ID, actrl, 
	...
	if ((dmabuf = IExec->AllocVec(buflen, MEMF_ANY|MEMF_PUBLIC|MEMF_CLEAR)) == NULL) 
	{
	...
	sample.ahisi_Length = buflen / IAHI->AHI_SampleFrameSize(type);

	if ((rc = IAHI->AHI_LoadSound(0, AHIST_DYNAMICSAMPLE, &sample, actrl)) != 0) 
	{
		Com_Printf("ERROR: Can't load sound\n");
		return qfalse;
	}
 
	if (IAHI->AHI_ControlAudio(actrl, 
					AHIC_Play, TRUE, 
				TAG_END) != 0) 
	{
...
void SNDDMA_Shutdown(void)
{
	...
	if (rc == 0 && actrl)
	{
		IAHI->AHI_UnloadSound(0, actrl);
		rc = 1;
	}

	if (dmabuf) 
	{
		IExec->FreeVec(dmabuf);
		dmabuf = NULL;
	}
