...
//                InstImm( "bc", PPC_BC, 4, 0, v );
	...
	Com_Printf( "VM file %s pass %d compiled to %i bytes of code\n", vm->name, (pass+1), compiledOfs*4 );
		...
		vm->codeBase = IExec->AllocVec(vm->codeLength, MEMF_EXECUTABLE);
#else
		...
		// Should flush the cache
			IExec->CacheClearE(vm->codeBase, vm->codeLength, CACRF_ClearI|CACRF_ClearD);
		#else
...
#if (defined(MACOS_X) || defined(__amigaos4__)) && defined(__OPTIMIZE__)
