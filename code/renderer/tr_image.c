...
image_t *R_CreateImage( const char *name, const byte *pic, int width, int height, qboolean mipmap, qboolean allowPicmip, int glWrapClampMode )
{
	...
	// 
	glState.currenttextures[glState.currenttmu] = 0; // fix ? - Cowcat
...
static void R_CreateDlightImage( void )
{
	...
	tr.dlightImage = R_CreateImage("*dlight", (byte *)data, DLIGHT_SIZE, DLIGHT_SIZE, qfalse, qfalse, GL_CLAMP /*_TO_EDGE*/ ); // Cowcat
}
...
static void R_CreateFogImage( void )
{
	...
	// what we want.
	tr.fogImage = R_CreateImage("*fog", (byte *)data, FOG_S, FOG_T, qfalse, qfalse, GL_CLAMP /*_TO_EDGE*/ ); // Cowcat
...
void R_CreateBuiltinImages( void )
{
	...
	for(x=0;x<32;x++)
	{
		...
		tr.scratchImage[x] = R_CreateImage("*scratch", (byte *)data, DEFAULT_SIZE, DEFAULT_SIZE, qfalse, qtrue, GL_CLAMP /*_TO_EDGE*/ ); // Cowcat
	}
...
{
	skinSurface_t	parseSurfaces[MAX_SKIN_SURFACES]; // new ioq3 - Cowcat
	qhandle_t	hSkin;
	skin_t		*skin;
	skinSurface_t	*surf;
	char		*text, *text_p;
	...
	while ( text_p && *text_p )
	{
		...
		surf = &parseSurfaces[skin->numSurfaces]; // new ioq3 - Cowcat
	...
	// copy surfaces to skin - new ioq3 - Cowcat
...
void R_SkinList_f( void )
{
	...
	for ( i = 0 ; i < tr.numSkins ; i++ )
	{
		...
		ri.Printf( PRINT_ALL, "%3i:%s (%d surfaces)\n", i, skin->name, skin->numSurfaces ); // new ioq3 - Cowcat
